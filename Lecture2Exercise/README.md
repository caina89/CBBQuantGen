...
* Usually after logging into a HPC we'd land in the `/home` partition of the computer we are in, that usually has very little storage space but is well backed-up. `/home` is usually abbreviated `~`. Usually it is not a great idea to put a lot of data in this space. Rather, this is where you set up your conda, and directories for your code `~/src` and the softwares you will use `~/bin`.
...
## Conda environment 
Usually for computational biology and bioinformatics projects Miniconda (lightweight) is sufficient. This installer is small and only includes conda and Python. You then install only the packages you need. You can install Miniconda in your `/home` directory. 
```
